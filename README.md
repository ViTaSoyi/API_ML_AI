...
**3.所使用的API类别的使用风险报告**  
- 市场竞争程度：目前国内文本纠错相关的API在自己能查询的条件下，有腾讯、百度、聚合数据和三色云四个平台提供文本纠错的接口服务。相对腾讯和百度，聚合和三色云成熟度与市场知名度较低，未见有该接口的相关合作案例。而目前腾讯文本纠错API仍处于公测期间，未有正式产品，成熟度、价格、优劣等各方面有待考量。因此，就个人研究而言，中文文本纠错接口服务目前在国内市场竞争程度不大。（在自己可查询范围下，未找到相关文献说明）
- 输入输出限制：待分析文本，长度有字节限制和编码要求。
