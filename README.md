...
**3.API使用风险报告**  
- 现在及未来发展性：文本纠错从互联网起始一直作为一些重要技术的辅助，譬如搜索引擎、火热的智能写作等。目前中文文本纠错基本分为三大步：错误识别、生成纠正候选和评价纠正候选。即使基于这样一个框架下，文本纠错目前也无法达到精准的准确率。语言千姿百态，一词/一句或有多重意思，机器尚未能够精准地理解语义，更何况精准地纠错文本，因此文本纠错在未来仍有极大的发展性。
- 市场竞争程度：目前国内文本纠错相关的API在自己能查询的条件下，有腾讯、百度、聚合数据和三色云四个平台提供文本纠错的接口服务。相对腾讯和百度，聚合和三色云成熟度与市场知名度较低，未见有该接口的相关合作案例。而目前腾讯文本纠错API仍处于公测期间，未有正式产品，成熟度有待考量。因此，就目前情况而言，中文文本纠错接口服务在国内市场竞争程度不大。
...
**4. AI产品概率性**  
在文本纠错上，对日常常用的固定词组和日常的常见的专有名词，文本纠错的成功率高；但对于古诗词和比较陌生的专有名词，文本纠错率会比较低。为此，在用户使用该功能时会弹出窗口，告诉用户“对于古诗词和比较陌生的专有名词，文本纠错率会比较低”。减少用户对该两类文本句子的文本纠错功能的使用，同时降低用户获得错误反馈或者无结果反馈的失望值。
...
- [百度文本翻译接口官方文档](http://fanyi-api.baidu.com/api/trans/product/apidoc#joinFile)  
